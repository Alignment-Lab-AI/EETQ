...
#pragma unroll
    for (int mask = 16; mask > 0; mask >>= 1)
        val = max(std::abs(val), __shfl_xor_sync(FINAL_MASK, val, mask, 32));  //__shfl_sync bf16 return float when sm < 80
    return val;
}
...
template<typename ComputeType, typename WeightType>
__global__ void quantize(int8_t* unprocessed_quantized_weight,
                    ComputeType* current_scale_ptr, 
                    const WeightType* __restrict current_weight, 
                    const size_t num_rows, const size_t num_cols){
    const int    bits_in_type = 8;
    const float  quant_range_scale = 1.f / float(1 << (bits_in_type - 1));
    float val = 0;
    
    for(int i = threadIdx.x; i < num_rows; i += blockDim.x){
      val = max(to_float(__ldg(&current_weight[i * num_cols + blockIdx.x])), val);
    }
    
    // First compute the maximum for each column
    ...
    for(int i = threadIdx.x; i < num_rows; i += blockDim.x){
      ...
      const float scaled_weight = round(weight / scale);
      const int8_t clipped_weight = int8_t(max(-128.f, min(127.f, scaled_weight)));
      unprocessed_quantized_weight[i * num_cols + blockIdx.x] = clipped_weight;
    }
}
                        ...
                        QuantType                  quant_type)
{
    ...
    for (int expert = 0; expert < num_experts; ++expert) {
        ...
        WeightType* device_current_weight;
        int8_t* device_current_quantized_weight;
        ComputeType* device_current_scales;
        cudaMalloc((void **)&device_current_weight, sizeof(WeightType) * num_cols * num_rows);
        cudaMalloc((void **)&device_current_quantized_weight, sizeof(int8_t) * num_cols * num_rows);
        cudaMalloc((void **)&device_current_scales, sizeof(ComputeType) * num_cols);
        dim3 n_thread = 256;
        dim3 n_blocks = num_cols;
        cudaMemcpy(device_current_weight, current_weight, sizeof(WeightType) * num_cols * num_rows, cudaMemcpyHostToDevice);
...
template void symmetric_quantize_launcher<half, float>(int8_t*, int8_t*, half*, const float*, const std::vector<size_t>&, QuantType);

template void symmetric_quantize_launcher<half, half>(int8_t*, int8_t*, half*, const half*, const std::vector<size_t>&, QuantType);

template void symmetric_quantize_launcher<float, float>(int8_t*, int8_t*, float*, const float*, const std::vector<size_t>&, QuantType);
